...
const ViewStudents = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const response = await fetch('http://localhost:8080/admin/viewAllStudents');
        if (!response.ok) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/admin/viewStudent/${searchId}`);
      if (!response.ok) {
  ...
  const handleViewDetails = (id) => {
    navigate(`/admin/fullstudent/${id}`); // Navigate to the full view page with student ID
  };
