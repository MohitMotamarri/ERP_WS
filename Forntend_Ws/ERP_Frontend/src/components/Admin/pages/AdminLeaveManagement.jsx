...
function AdminLeaveManagement() {
    ...
    const fetchLeaveRequests = async () => {
        try {
            setLoading(true);
            const response = await axios.get('http://localhost:8080/faculty/appliedforleave');
    ...
    const handleStatusUpdate = async (leaveId, status) => {
        ...
        try {
            await axios.put(`http://localhost:8080/faculty/update/${leaveId}?status=${status}`);
                            ...
                            leaveRequests.map((request) => (
                                <tr key={request.leaveId} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {request.faculty && request.faculty.facultyName ? request.faculty.facultyName : 'No Faculty'}
                                    </td>
                                    ...
                                    {request.status === 'Pending' && (
                                            <div className="flex space-x-2">
                                                <button
                                                    onClick={() => handleStatusUpdate(request.leaveId, 'Accepted')}
